...
try {
    ...
    // Convert events to InnoDB
    $db->exec("ALTER TABLE events ENGINE=InnoDB");
    echo "Converted 'events' to InnoDB.\n";

    // Check users table existence and engine
    try {
        $stmt = $db->query("SHOW CREATE TABLE users");
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        if (strpos($result['Create Table'], 'MyISAM') !== false) {
            $db->exec("ALTER TABLE users ENGINE=InnoDB");
            echo "Converted 'users' to InnoDB.\n";
        }
    } catch (Exception $e) {
        echo "Users table might not exist or error: " . $e->getMessage() . "\n";
    }

    // Now create registrations table
    $sql = "
